...
### ~~`segmentos`~~ *(ELIMINADA — refactor 2026-07-01)*
Colección y modelo eliminados. La asignación granular absorbió su rol: cada `Asignacion` es ahora un registro persona↔turno completo (sin sub-buckets de 30 min).

### `asignaciones` *(salida del algoritmo — registro persona↔turno)*
```
id, empresa_id, ubicacion_id, empleado_id, date,
estacion_id|null (null = congregación),
start: HH:MM (= turno.start_time), end: HH:MM (= turno.end_time),
status: 'sugerido'|'publicado',
active, timestamps, deletedAt
```
Índices: `(ubicacion_id, date, deletedAt)`, `(empleado_id, date, status, deletedAt)`, `(ubicacion_id, date, status, deletedAt)`.

**Regla D4 (invariante dura):** un empleado no puede aparecer más de una vez en el mismo turno ni en turnos solapados el mismo día. Enforceada en la Cloud Function antes de cualquier otro criterio.

**Regla D5:** asignaciones `publicado` solo pueden modificarse por `super_admin`, `company_admin`, `branch_manager` (o `owner`); enforceada en Security Rules.
...
1. **Demanda de cobertura** = `ConfiguracionTurnos` de la ubicación, resuelta por precedencia `range > month > default`. Plantilla anual + overrides por mes/semana/caso especial. Bloques `{estacion_id, day_of_week, start_time, end_time, cantidad}`. Turnos solapados = cobertura escalonada.
2. **Oferta del día** (derivada, en memoria): `disponibilidad − excepciones`. Fuente base = `Empleado.disponibilidad.ventanas` (`{ day_of_week, start, end }[]`); se le **restan** las `excepciones` del empleado ese día. Aplica a empresa y congregación.

### Algoritmo (CF `generarAsignaciones` — refactor 2026-07-01)
- **Tipo:** **greedy** + edición manual (no solver). CF `onCall` región `southamerica-west1`.
- **Entrada:** demanda (ConfiguracionTurnos) + oferta (`disponibilidad − excepciones`) + `estacion_ids` + reglas.
- **Salida:** `asignaciones` (una por empleado↔turno) en `sugerido`. **Sin sub-buckets de 30 min.**
- **D4 (invariante dura):** un empleado no puede repetirse en el mismo turno ni en turnos solapados el mismo día. Se verifica **antes** de cualquier otro criterio.
- **Idempotencia:** antes de insertar, se soft-delete todos los `sugerido` del día. Los `publicado` no se tocan.
- **Equidad:** equidad por minutos asignados en la semana + tiebreaker aleatorio (variación entre regeneraciones).
- **Anti-saturación (solo empresa):** respetar `max_continuo_min`; el empleado saturado se saltea.
- **Reglas hard:** `reglas_asignacion` con `is_strict + type='nunca_juntos'` (solo empresa).
- **Cobertura incumplible:** se marca **hueco visible**, no se inventa gente (restricción dura).
- **Logs en respuesta:** la CF devuelve `logs: string[]` que el cliente vuelca al `logStore` para diagnóstico.

### Publicación, calendario y solicitudes
- `sugerido → publicado`. Los `publicado` solo los puede modificar el manager/super_admin (D5).
- El empleado **ve su calendario** (solo `publicado`). Se carga con **lectura puntual** al abrir la vista.
- **Excepciones = `solicitudes`** con estados. No hay recálculo automático.

### Disparadores de regeneración
- Se guarda o edita una `ConfiguracionTurnos` → `TurnosView` llama `regenerarSugerenciasSilencioso`.
- Se guarda `Empleado.disponibilidad` → `AjustesDisponibilidadView` llama ídem.
- Se crea/elimina una `Excepcion` → `AjustesExcepcionesView` llama ídem.
- El manager abre el Calendario → llama explícitamente con botón "Generar sugerencias".

### Caso congregación (reuso del motor — decisión fijada)

La congregación **no tiene modelos propios**: mismo tenant `Empresa` (`type: 'congregacion'`, `facturable: false`).
...
| Presencia derivada | Disponibilidad | Lectura directa de `Empleado.disponibilidad` |
| `Estacion` | — (no aplica) | Demanda = `cantidad` con `estacion_id = null` |
| `Asignacion` | Igual | `estacion_id = null` |
| `reglas_asignacion`, `intensidad`, `max_continuo`, colación, facturación/DTE | — | **OFF** por `type` |

**Algoritmo (rama congregación):** para cada turno del día, tomar `requerimiento.cantidad` voluntarios cuya disponibilidad contenga el turno completo; D4 aplica; equidad + tiebreaker aleatorio; hueco si falta gente.
...
| R1 | **Refactor asignaciones + algoritmo + consola logs** | ✅ | Desplegar: `firebase deploy --only functions,firestore:indexes` |

> **Estado actual:** Fases 0–12, Revisión integral y Refactor R1 cerrados. **Pendiente despliegue:** `firebase deploy --only functions,firestore:indexes`.
...
### R1 — Refactor asignaciones + algoritmo + consola logs ✅ (2026-07-01)
- **Hecho:**
  - `src/models/Asignacion.ts`: reescrito. Ahora representa un registro persona↔turno completo: añade `empleado_id`, `estacion_id|null`, `start`, `end`. `AsignacionStatus = 'sugerido'|'publicado'` (eliminados `draft`, `aprobado`, `rechazado`). `normalizeStatus` para retrocompatibilidad.
  - `src/models/Segmento.ts` + `src/stores/segmentoStore.ts`: **eliminados** del proyecto.
  - `src/stores/asignacionStore.ts`: reescrito con API completa: `cargarAsignacionesManager`, `cargarAsignacionesEmpleado`, `publicarDia`, `softDeleteAsignacion`, `crearAsignacion`, `actualizarEmpleadoAsignacion`, `regenerarSugerencias` (llama CF `generarAsignaciones`), `regenerarSugerenciasSilencioso` (fire-and-forget).
  - `src/stores/logStore.ts`: nuevo. Buffer de 500 entradas, niveles debug/info/warn/error, fuentes client/fn, `pushServerLogs`, estado open/minimized.
  - `src/components/LogConsole.vue`: nuevo. Panel fijo bottom (left: sidebar offset), minimizable, expandible, resize por drag, filtros por nivel y fuente, auto-scroll.
  - `src/App.vue`: monta `<LogConsole />`.
  - `src/views/sucursal/CalendarioView.vue`: reescrito sin buckets ni segmentos. Grilla semanal (empresa) y mensual (congregación). 2 estados: sugerido/publicado. Diagnóstico integrado. Sidebar de reasignación con filtro D4 (no repite empleado en turnos solapados). Carga puntual sin listeners.
  - `src/views/sucursal/PersonalView.vue`, `AjustesDisponibilidadView.vue`, `AjustesExcepcionesView.vue`: reemplazadas las llamadas a `segmentoStore.regenerarBorradorMes` por `asignacionStore.regenerarSugerenciasSilencioso` con callbacks de log.
  - `functions/src/index.ts`: reescrito. `generarBorrador` y `actualizarBorrador` (dead code) eliminados. Nueva CF `generarAsignaciones` (escribe a colección `asignaciones`, D4 hard rule, idempotencia, equidad + tiebreaker aleatorio, logs en respuesta, try/catch completo). Facturación intacta.
  - `firestore.rules`: bloque `segmentos` y `presencias` eliminados; `asignaciones` actualizado con regla D5 (publicados solo super_admin/manager pueden modificar).
  - `firestore.indexes.json`: índices de `segmentos` eliminados; 3 índices de `asignaciones` agregados.
  - `tsc` 0 (frontend) y `tsc` 0 (functions).
- **Pendiente despliegue:** `firebase deploy --only functions,firestore:indexes`.