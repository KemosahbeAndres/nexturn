  ...
  <div class="flex h-[100dvh] bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      <PageHeader
        ...
        :subtitle="String(route.meta.subtitle || ubicacion?.name || 'Sucursal')"
      />
...
watchEffect(() => {
  ...
  if (ubicacionSlug && lista && lista.length > 0) {
    const found = lista.find(u => {
      const efectiveSlug = u.slug || slugify(u.name);
      return efectiveSlug === ubicacionSlug;
    });
