<template>
  <div class="flex h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
    ...
    <aside class="hidden md:flex md:w-44 lg:w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex-col shrink-0 transition-[width] duration-300">
      ...
      <!-- Logo -->
      <div class="h-16 flex items-center px-3 lg:px-6 border-b border-gray-200 dark:border-gray-700">
        <h1 class="text-base lg:text-xl font-bold text-gray-900 dark:text-white truncate">Nexturn</h1>
      </div>

      <!-- Perfil -->
      <div class="p-3 lg:p-6 border-b border-gray-200 dark:border-gray-700">
        <p class="text-xs lg:text-sm font-semibold text-gray-900 dark:text-white truncate">
      ...
      <!-- Navegación -->
      <nav class="flex-1 px-2 lg:px-4 py-3 lg:py-4 space-y-0.5 lg:space-y-1 overflow-y-auto">
        <router-link
      ...
      <!-- Pie -->
      <div class="p-2 lg:p-4 border-t border-gray-200 dark:border-gray-700 space-y-0.5 lg:space-y-1 shrink-0">
        <router-link
          v-if="sessionStore.userRole === 'super_admin'"
          to="/dashboard"
          class="w-full flex items-center gap-1.5 lg:gap-2 px-2 lg:px-3 py-2 text-xs lg:text-sm font-medium text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors"
        >
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      <header class="md:hidden shrink-0 h-14 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-4 gap-2 transition-colors duration-300">
        <!-- Botón "Panel Admin" solo cuando es super_admin -->
        <router-link
          v-if="sessionStore.userRole === 'super_admin'"
          to="/dashboard"
