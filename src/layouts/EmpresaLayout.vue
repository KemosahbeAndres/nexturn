<template>
  <div class="flex h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
    ...
    <!-- ===================== SIDEBAR (md+) ===================== -->
    <aside class="hidden md:flex w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex-col shrink-0">
      ...
      <!-- Pie -->
      <div class="p-4 border-t border-gray-200 dark:border-gray-700 space-y-1 shrink-0">
        <router-link
          v-if="sessionStore.userRole === 'super_admin'"
          to="/dashboard"
          class="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors"
        >
        ...
        <button
          ...
          class="w-full flex items-center justify-between px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors"
        >
          <div class="flex items-center gap-2 text-sm font-medium text-gray-600 dark:text-gray-300">
            <svg v-if="!sessionStore.isDarkMode" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 text-amber-500">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0z" />
            </svg>
            <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 text-blue-400">
              <path stroke-linecap="round" stroke-linejoin="round" d="M21.752 15.002A9.72 9.72 0 0 1 18 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 0 0 3 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 0 0 9.002-5.998z" />
            </svg>
            <span>{{ sessionStore.isDarkMode ? 'Modo oscuro' : 'Modo claro' }}</span>
          </div>
          <div class="relative w-10 h-5 rounded-full transition-colors duration-300" :class="sessionStore.isDarkMode ? 'bg-blue-500' : 'bg-gray-300'">
            <span class="absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform duration-300" :class="sessionStore.isDarkMode ? 'translate-x-5' : 'translate-x-0'"></span>
          </div>
        </button>
        <button
          @click="handleLogout"
          class="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium text-red-600 dark:text-red-400 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 shrink-0">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      <header class="md:hidden shrink-0 h-14 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-4 transition-colors duration-300">
        <span class="text-base font-semibold text-gray-800 dark:text-white truncate">{{ currentPageTitle }}</span>
        <router-link
          :to="{ name: 'empresa-ajustes', params: { companySlug: $route.params.companySlug } }"
          class="w-9 h-9 rounded-full bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center shrink-0"
        >
...
// ---- Iconos inline como componentes para poder usar :is="item.icon" ----
...
const menuItems = [
  { name: 'Dashboard',  routeName: 'empresa-dashboard',  icon: IconDashboard },
  { name: 'Usuarios',   routeName: 'empresa-usuarios',   icon: IconUsuarios },
  { name: 'Personal',   routeName: 'empresa-personal',   icon: IconPersonal },
  { name: 'Sucursales', routeName: 'empresa-sucursales', icon: IconSucursales },
  { name: 'Turnos',     routeName: 'empresa-turnos',     icon: IconTurnos },
  { name: 'Calendario', routeName: 'empresa-calendario', icon: IconCalendario },
];

// Bottombar: los 5 más importantes (sin Usuarios para ahorrar espacio)
const bottomItems = [
  { name: 'Inicio',     routeName: 'empresa-dashboard',  icon: IconDashboard },
  { name: 'Personal',   routeName: 'empresa-personal',   icon: IconPersonal },
  { name: 'Turnos',     routeName: 'empresa-turnos',     icon: IconTurnos },
  { name: 'Calendario', routeName: 'empresa-calendario', icon: IconCalendario },
  { name: 'Sucursales', routeName: 'empresa-sucursales', icon: IconSucursales },
...
const currentPageTitle = computed(() => {
  const found = menuItems.find(item => item.routeName === route.name);
  return found?.name || activeCompany.value;
});
