<template>
  <div class="max-w-2xl mx-auto space-y-6">
    ...
    <!-- Avatar + nombre -->
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6 flex items-center gap-4">
      <div class="w-16 h-16 rounded-full bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center shrink-0 text-blue-600 dark:text-blue-300 text-2xl font-bold select-none">
        {{ initials }}
      </div>
      <div class="min-w-0">
        <p class="text-lg font-semibold text-gray-900 dark:text-white truncate">{{ fullName }}</p>
        <span :class="roleBadgeClass" class="inline-block mt-1 px-2.5 py-0.5 text-xs font-semibold rounded-md capitalize">
    ...
    <!-- Formulario -->
    <form @submit.prevent="handleSave" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6 space-y-4">
      <h2 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Información personal</h2>
