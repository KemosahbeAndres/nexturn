        ...
        <div class="px-4 py-3 flex items-start justify-between gap-2 border-b border-gray-50 dark:border-gray-700/50">
          <div class="min-w-0 flex-1">
            ...
            <div class="flex flex-wrap gap-x-3 mt-1">
              ...
              <p v-if="nombreManager(ub.manager_id)" class="text-xs text-gray-400 dark:text-gray-500">
                <span class="font-medium">Encargado:</span> {{ nombreManager(ub.manager_id) }}
              </p>
            </div>
          </div>
      ...
      <div v-if="modalOpen" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
        ...
        <div class="relative w-full sm:max-w-lg bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[90dvh] overflow-y-auto">
          ...
          <form @submit.prevent="guardar" class="p-5 space-y-4">
            ...
            <div>
              <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Rol requerido <span class="text-red-500">*</span></label>
            ...
            <div>
              <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                Encargado <span class="text-gray-400 font-normal">(opcional)</span>
              </label>
              <select v-model="form.manager_id"
                :disabled="!form.required_role"
                class="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed">
                <option value="">Sin encargado</option>
                <option v-for="emp in personalConRol" :key="emp.id" :value="emp.id">
                  {{ emp.displayName }}
                </option>
              </select>
              <p v-if="form.required_role && personalConRol.length === 0" class="mt-1 text-xs text-gray-400 dark:text-gray-500">
                No hay personal con el rol "{{ form.required_role }}" registrado.
              </p>
              <p v-else-if="!form.required_role" class="mt-1 text-xs text-gray-400 dark:text-gray-500">
                Selecciona el rol requerido primero.
              </p>
            </div>
...
// Personal filtrado por el rol requerido seleccionado en el formulario
const personalConRol = computed(() => {
  if (!form.value.required_role) return [];
  return (empleadoStore.empleadosActivos ?? []).filter(
    e => e.work_role === form.value.required_role
  );
});
...
function nombreManager(managerId: string | null): string {
  if (!managerId) return '';
  return empleadoStore.empleados?.find(e => e.id === managerId)?.displayName ?? '';
}
...
function abrirModalEditar(ub: Ubicacion) {
  // required_role: tomamos el primero de los turnos activos si existe, o vacío
  const primerRol = ub.activeTurnos[0]?.required_roles[0] ?? '';
  form.value = {
    name: ub.name,
    address: ub.address,
    category: ub.category,
    zone_id: ub.zone_id ?? '',
    required_role: primerRol,
    manager_id: ub.manager_id ?? '',
    active: ub.active,
  };
