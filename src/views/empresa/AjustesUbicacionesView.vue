<template>
  <div class="p-4 sm:p-6 lg:p-8 space-y-6">
    <p class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Ubicaciones / Sucursales</p>
    ...
    <div class="flex flex-col sm:flex-row gap-3">
      <div class="relative flex-1">
        ...
        <input v-model="busqueda" type="search" placeholder="Buscar por nombre o dirección..."
    ...
    <div v-if="ubicacionesFiltradas.length === 0 && ubicacionesActivas.length === 0"
      ...
      <p class="text-sm font-medium text-gray-500 dark:text-gray-400">
        {{ busqueda || filtroCategoria ? 'Sin resultados.' : 'No hay ubicaciones registradas aún.' }}
      </p>
      <button v-if="!busqueda && !filtroCategoria" @click="abrirModalCrear"
        class="mt-3 text-sm font-medium text-blue-600 dark:text-blue-400 hover:underline">
        Agregar la primera
      </button>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      <article v-for="ub in ubicacionesFiltradas" :key="ub.id"
        ...
        <div class="px-4 py-3 flex items-start justify-between gap-2 border-b border-gray-50 dark:border-gray-700/50">
          ...
          <div class="flex items-center gap-1 shrink-0">
            <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium"
              :class="ub.active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' : 'bg-red-100 text-red-600 dark:bg-red-900/40 dark:text-red-300'">
        ...
        <div class="px-4 py-3">
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">Turnos ({{ ub.activeTurnos.length }})</p>
          <div v-if="ub.activeTurnos.length === 0" class="text-xs text-gray-400 dark:text-gray-500 italic">Sin turnos configurados</div>
      ...
      <div v-if="modalOpen" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
        ...
        <div class="relative w-full sm:max-w-lg bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[90dvh] overflow-y-auto">
          ...
          <form @submit.prevent="guardar" class="p-5 space-y-4">
            ...
            <div>
              <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Slug (URL)</label>
              <input
                :value="form.slug"
                @input="form.slug = ($event.target as HTMLInputElement).value; onSlugInput()"
                @focus="onSlugFocus"
                @blur="onSlugBlur"
                type="text"
                placeholder="se-genera-automaticamente"
                class="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono"
              />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p v-if="zonasActivas.length === 0" class="mt-1 text-xs text-amber-500">No hay zonas creadas aún.</p>
              </div>
            </div>
...
const zonasActivas = computed(() => (zonaStore.zonas ?? []).filter(z => z.active));
const ubicacionesActivas = computed(() => (ubicacionStore.ubicaciones ?? []).filter(u => u.active));
...
const ubicacionesFiltradas = computed(() => {
  ...
  if (busqueda.value.trim()) {
    ...
    lista = lista.filter(u => u.name.toLowerCase().includes(q) || (u.address ?? '').toLowerCase().includes(q));
  }
  return lista;
});
...
const formVacio = () => ({
  name: '', address: '', category: '' as UbicacionCategory,
  zone_id: '', active: true, slug: '',
});
const form = ref(formVacio());
const slugEditadoManualmente = ref(false);

watch(() => form.value.name, (nuevoNombre) => {
  if (!slugEditadoManualmente.value) {
    form.value.slug = slugify(nuevoNombre);
  }
});
...
function abrirModalEditar(ub: Ubicacion) {
  const slugActual = ub.slug || slugify(ub.name);
  form.value = {
    name: ub.name, address: ub.address, category: ub.category,
    zone_id: ub.zone_id ?? '', active: ub.active, slug: slugActual,
  };
  slugEditadoManualmente.value = true; // en edición el slug no debe sobreescribirse automáticamente
  ...
  try {
    if (modoEdicion.value && ubicacionSeleccionada.value) {
      await ubicacionStore.updateUbicacion(ubicacionSeleccionada.value.id, {
        name: form.value.name, address: form.value.address, category: form.value.category,
        zone_id: form.value.zone_id || null, active: form.value.active,
