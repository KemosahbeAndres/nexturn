<template>
  <div class="space-y-4">
    <p class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Roles de trabajo</p>
    <p class="text-xs text-gray-500 dark:text-gray-400">
      Roles que pueden asignarse al personal. Los roles secundarios heredan la jerarquía del rol padre.
    </p>

    <!-- Árbol de roles -->
    <div v-if="empresa.cargos?.length" class="space-y-1">
      <RoleTreeItem
    ...
    <!-- Formulario agregar -->
    <form v-if="canManage" @submit.prevent="addRole" class="space-y-3 pt-4 border-t border-gray-100 dark:border-gray-700">
      <p class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Agregar rol</p>
      ...
      <div class="flex gap-2">
        ...
        >
          ...
          <option v-for="r in empresa.cargos" :key="r.id" :value="r.id">
            {{ r.nombre }}
          </option>
        </select>
        <button
          ...
          class="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors whitespace-nowrap"
        >
          + Agregar
        </button>
      </div>
...
const rolesRaiz = computed(() =>
  (props.empresa.cargos ?? []).filter(r => r.parent_role === null)
);
